...
import { MOCK_SLEEP_HISTORY, MOCK_MARKETS, MOCK_DEVICES, USER_PROFILE, MOCK_RESOURCES } from '@/constants';
...
const LoadingState: React.FC<{ text?: string }> = ({ text = "LOADING..." }) => (
  <div className="flex flex-col items-center justify-center py-10 animate-fade-in">
    <div className="relative w-12 h-12 mb-4">
      <div className="absolute inset-0 rounded-full border-4 border-surfaceHighlight"></div>
      <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-sport animate-spin"></div>
    </div>
    <span className="text-xs font-mono font-bold text-sport tracking-[0.2em] animate-pulse">{text}</span>
  </div>
);
...
const SuccessState: React.FC<{ text?: string; subtext?: string; onDismiss: () => void }> = ({ text = "SUCCESS", subtext, onDismiss }) => (
  <div className="flex flex-col items-center justify-center py-10 animate-fade-in">
    <div className="w-16 h-16 bg-sport rounded-full flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(204,255,0,0.4)] animate-[slideUp_0.4s_cubic-bezier(0.175,0.885,0.32,1.275)]">
      <Check className="w-8 h-8 text-black stroke-[3]" />
    </div>
    <h3 className="text-xl font-display font-bold uppercase text-white mb-2">{text}</h3>
    {subtext && <p className="text-xs text-gray-400 font-mono mb-6 text-center">{subtext}</p>}
    <button 
      onClick={onDismiss}
      className="bg-surfaceHighlight border border-border px-6 py-2 rounded-sm text-xs font-bold uppercase hover:bg-surface hover:text-white transition-colors"
    >
      Close
    </button>
  </div>
...
export default function Page() {
  ...
  const [connectedDevices, setConnectedDevices] = useState<Device[]>(MOCK_DEVICES);
  
  // Profile State
  const [userProfile, setUserProfile] = useState<UserProfile>(USER_PROFILE);
  const [isEditingProfile, setIsEditingProfile] = useState(false);
  const [tempProfile, setTempProfile] = useState<UserProfile>(USER_PROFILE);
  ...
  useEffect(() => {
    if (!isOnboarding) {
      const loadInsight = async () => {
        const today = MOCK_SLEEP_HISTORY[MOCK_SLEEP_HISTORY.length - 1];
  ...
  const todaySleep = MOCK_SLEEP_HISTORY[MOCK_SLEEP_HISTORY.length - 1];
  ...
  const handlePlaceBet = () => {
    ...
    setTimeout(() => {
        const newBet: UserBet = {
          ...
          entryPrice: betPosition === 'YES' ? showBetModal.yesPercent : showBetModal.noPercent,
          potentialPayout: betAmount * (100 / (betPosition === 'YES' ? showBetModal.yesPercent : showBetModal.noPercent)),
          status: 'OPEN'
        };
        setActiveBets([...activeBets, newBet]);
        setIsLoading(false);
        setIsSuccess(true); // Trigger success state
    }, 2000); // 2 second mock transaction
  };
  ...
  const toggleDevice = (id: string) => {
    setConnectedDevices(prev => prev.map(d => 
      d.id === id ? { ...d, connected: !d.connected, lastSync: !d.connected ? new Date() : d.lastSync } : d
    ));
  };
  ...
  const renderDashboard = () => (
    <div className="space-y-6 pb-28 animate-slide-up pt-4">
      ...
      <div className="glass-card p-4 rounded-sm">
        ...
        <SleepChart data={MOCK_SLEEP_HISTORY} />
      </div>
    </div>
  );
      ...
      {marketSubTab === 'EXPLORE' ? (
        <>
          ...
          <div className="space-y-3">
              {MOCK_MARKETS.length > 0 ? MOCK_MARKETS.map(market => (
              <div 
                  key={market.id} 
        ...
        <div className="space-y-4">
           ...
           {activeBets.length > 0 ? (
             activeBets.map((bet, idx) => {
               ...
               const market = MOCK_MARKETS.find(m => m.id === bet.marketId);
  ...
  const renderLeaderboard = () => (
    <div className="space-y-6 pb-28 animate-slide-up pt-14">
      ...
      <div className="glass-card rounded-sm overflow-hidden">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className={`p-4 flex items-center justify-between border-b border-border last:border-0 ${i === 4 ? 'bg-sport/5 border-l-2 border-l-sport' : ''}`}>
            <div className="flex items-center gap-4">
              <span className={`font-mono w-6 text-sm ${i <= 3 ? 'text-sport' : 'text-gray-500'}`}>{i === 4 ? USER_PROFILE.rank : i + 3}</span>
              <div className="flex flex-col">
                <span className={`font-bold text-sm ${i === 4 ? 'text-white' : 'text-gray-300'}`}>
                  {i === 4 ? USER_PROFILE.username.toUpperCase() : `USER_ID_88${i}`}
                </span>
  ...
  const renderProfile = () => {
    ...
    const displayAvatar = isEditingProfile ? tempProfile.avatarUrl : userProfile.avatarUrl;
      ...
      <div className="space-y-6 pb-28 animate-slide-up pt-14">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="glass-card p-4 rounded-sm text-center border-t-2 border-t-secondary">
            <p className="text-gray-500 text-[10px] font-mono uppercase mb-1">Portfolio Value</p>
            <p className="text-2xl font-mono font-bold text-white flex justify-center items-center gap-1">
               <Wallet className="w-4 h-4 text-gray-500" />
               {userProfile.solBalance}
            </p>
          </div>
        ...
        <>
          ...
          {showWalletModal && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
              {isLoading ? <LoadingState text="CONNECTING..." /> : (
                  <div className="glass-card w-full max-w-sm rounded-sm p-6 border border-border shadow-2xl">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold tracking-wide">PHANTOM</span>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold tracking-wide text-gray-400">SOLFLARE</span>
                        </div>
                        </button>

                         <button 
                        onClick={() => handleConnectWallet('Passkey')}
                        className="w-full bg-surface hover:bg-surfaceHighlight p-4 rounded-sm flex items-center justify-between group transition-colors border border-border"
                        >
                        <div className="flex items-center gap-3">
                            <Fingerprint className="w-5 h-5 text-white" />
                            <span className="font-bold tracking-wide text-gray-400">PASSKEY</span>
                        </div>
                        </button>
                      </div>
                  </div>
  ...
  return (
    <div className="min-h-screen font-sans bg-dark text-gray-200 selection:bg-sport/30 overflow-hidden relative">
      ...
      {/* Main Content Area */}
      <main className="relative z-10 max-w-md mx-auto h-screen overflow-y-auto overflow-x-hidden">
      ...
      {/* Sporty Bottom Nav */}
      <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pb-6 pt-4 bg-gradient-to-t from-black via-black/90 to-transparent">
        <div className="glass-panel px-4 py-3 rounded-full flex gap-6 shadow-2xl border border-border backdrop-blur-xl">
      ...
      {showWalletDetails && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
             <div className="glass-card w-full max-w-sm rounded-sm p-6 border border-border shadow-2xl">
                 ...
                 <div className="bg-surfaceHighlight p-4 rounded-sm mb-6 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold font-mono text-white">8x72...3f29</div>
                    </div>
                 </div>
                 <button 
      ...
      {/* Betting Modal (Redesigned) */}
      {showBetModal && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4 bg-black/80 backdrop-blur-md">
            {isLoading ? (
                <div className="glass-panel w-full sm:max-w-md rounded-t-xl sm:rounded-xl p-8 border border-sport/20 shadow-2xl animate-slide-up bg-[#09090b]">
                   <LoadingState text="CONFIRMING ORDER..." />
                </div>
            ) : isSuccess ? (
                <div className="glass-panel w-full sm:max-w-md rounded-t-xl sm:rounded-xl p-8 border border-green-500/30 shadow-2xl animate-slide-up bg-[#09090b]">
                   <SuccessState 
                     text="Order Filled" 
                ...
                <div className="glass-panel w-full sm:max-w-md rounded-t-xl sm:rounded-xl border border-border shadow-2xl animate-slide-up bg-[#09090b] flex flex-col max-h-[90vh]">
                    {/* Header */}
...
const NavButton: React.FC<{ icon: React.ReactNode; label: string; active: boolean; onClick: () => void }> = ({ icon, label, active, onClick }) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center gap-1 transition-all duration-200 group relative ${active ? 'text-sport' : 'text-gray-600 hover:text-gray-400'}`}
  >
    {React.cloneElement(icon as React.ReactElement<any>, { size: 18, strokeWidth: active ? 2.5 : 2 })}
    <span className={`text-[8px] font-bold font-mono tracking-widest ${active ? 'opacity-100' : 'opacity-0'} absolute -bottom-4 whitespace-nowrap transition-opacity`}>
      {label}
    </span>
    {active && <div className="absolute -top-3 w-1 h-1 rounded-full bg-sport shadow-[0_0_5px_#ccff00]"></div>}
  </button>
);

// Page component exported above